  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 left-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md glass-panel border-r border-brand-gold/30 shadow-2xl flex flex-col justify-between">
          {/* Header */}
          <div className="p-4 border-b border-brand-gold/20 flex items-center justify-between bg-brand-dark-900/90">
            <div className="flex items-center gap-2">
              <ShoppingBag className="w-6 h-6 text-brand-gold" />
              <h2 className="font-black text-lg text-brand-cream">سلة طلباتك 🍽️</h2>
              <span className="text-xs bg-brand-gold text-brand-dark-900 font-extrabold px-2 py-0.5 rounded-full">
          ...
          {selectedBranch && (
            <div className="px-4 py-2 bg-brand-dark-800/80 border-b border-brand-dark-700 flex items-center justify-between text-xs">
              <div className="flex items-center gap-1.5 text-brand-cream-muted">
                <MapPin className="w-3.5 h-3.5 text-brand-gold" />
                <span>الفرع المختار:</span>
                <span className="font-bold text-brand-cream">{selectedBranch.name_ar}</span>
              </div>
            </div>
          )}
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex-1 space-y-1">
                      <h4 className="font-bold text-sm text-brand-cream leading-tight">
                        {item.product.name_ar}
                      </h4>
                      {item.selected_options && item.selected_options.length > 0 && (
                        <p className="text-[11px] text-brand-gold leading-normal">
          ...
          {cart.length > 0 && (
            <div className="p-4 bg-brand-dark-900 border-t border-brand-gold/20 space-y-3">
              ...
              <div className="space-y-1 text-xs">
                ...
                <div className="flex justify-between text-sm font-black text-brand-cream pt-2 border-t border-brand-dark-700">
